...
class Ybc_blog_free extends Module
{
    ...
    private function getProductInfo($id_product, $id_lang = false)
    {
        ...
        $pinfo['price'] = Tools::displayPriceSmarty(['price' => $price], $this->context->smarty);       
        $pinfo['old_price'] = Tools::displayPriceSmarty(['price' => $oldPrice], $this->context->smarty); 
        $pinfo['discount_percent'] = round(($oldPrice - $price) / $oldPrice * 100);
        $pinfo['discount_amount'] = Tools::displayPriceSmarty(['price' => $discount], $this->context->smarty);
            ...
            elseif ($items)
            {
            	...
            	foreach ($items AS $item)
            	{
            		$product = array(
            			...
            			'image' => str_replace('http://', Tools::getShopProtocol(), Context::getContext()->link->getImageLink($item['link_rewrite'], $item['id_image'], $this->is17 ? ImageType::getFormattedName('home') : ImageType::getFormatedName('home'))),
            		);
