...
class MCAPI {
    ...
    */
    ...
    public function callServer($method, $params) {
        ...
        // Validate response before attempting to unserialize
        if (empty($response)) {
            $response = array("error" => "Empty response from MailChimp API", "code" => "-99");
        } else {
            // Check if response starts with expected serialized data format
            $response = trim($response);
            if (!preg_match('/^(a:|s:|i:|b:|d:|O:|N;)/', $response)) {
                // Response is not serialized data, possibly HTML error or JSON
                $response = array("error" => "Invalid response format. Got: " . substr($response, 0, 200), "code" => "-99");
            } else {
                // Attempt to unserialize with error suppression
                $serial = @unserialize($response);
                if($response && $serial === false) {
                    $response = array("error" => "Failed to unserialize response. Got: " . substr($response, 0, 200), "code" => "-99");
                } else {
                    $response = $serial;
                }
            }
