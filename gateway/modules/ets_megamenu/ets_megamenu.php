...
{    
        ...
        if(isset($menu['link_type']))
        {
            switch($menu['link_type'])
            {
                ...
                case 'MNFT':
                    ...
                    {
                        if ((int)Configuration::get('PS_REWRITING_SETTINGS'))
    						$manufacturer->link_rewrite = Tools::str2url($manufacturer->name);
    					else
    ...
    public function getProducts($ids, $order_by = false)
    {
        ...
        $sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) AS quantity'.(Combination::isFeatureActive() ? ', IFNULL(product_attribute_shop.id_product_attribute, 0) AS id_product_attribute,
					...
					pl.`available_later`, pl.`link_rewrite`, pl.`meta_description`' . (version_compare(_PS_VERSION_, '9.0.0', '<') ? ', pl.`meta_keywords`' : '') . ', pl.`meta_title`, pl.`name`, image_shop.`id_image` id_image,
        ...
        if (version_compare(_PS_VERSION_, '9.0.0', '>=') && Tools::getValue('configure')=='ets_megamenu') {
            foreach ($products as &$product) {
                $product['link'] = $this->context->link->getProductLink($product['id_product'], $product['link_rewrite'], null, null, $id_lang);
            }
        }
    ...
    private function importXmlConfig($xml)
    {
        ...
        foreach(self::$configs['configs'] as $key => $config)
        {
            if(property_exists($xml,$key))
            {
                ...
                {
                    $node = $xml->$key;
                    Configuration::updateValue($key,isset($node['configValue']) && (string)$node['configValue'] ? (string)$node['configValue'] : (isset($config['default']) ? $config['default'] : ''));
                }                   
            }
        }
    }
