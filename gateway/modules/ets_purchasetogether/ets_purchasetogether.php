...
class Ets_purchasetogether extends Module
{
    ...
    public function assignContentVars($params)
    {
        ...
        $shipping_cost = Tools::displayPriceSmarty(['price' => $base_shipping, 'currency' => $currency->id], $this->context->smarty);
        ...
        if ($useTax && Configuration::get('PS_TAX_DISPLAY') == 1) {
            ...
            $this->smarty->assign('tax_cost', Tools::displayPriceSmarty(['price' => $totalToPay - $totalToPayWithoutTaxes, 'currency' => $currency->id], $this->context->smarty));
        }

        // The cart content is altered for display
        foreach ($cart_rules as &$cart_rule) {
            if ($cart_rule['free_shipping']) {
                $shipping_cost = Tools::displayPriceSmarty(['price' => 0, 'currency' => $currency->id], $this->context->smarty);
        ...
        $this->smarty->assign(array(
            ...
            'wrapping_cost' => Tools::displayPriceSmarty(['price' => $wrappingCost, 'currency' => $currency->id], $this->context->smarty),
            'product_total' => Tools::displayPriceSmarty(['price' => $params['cart']->getOrderTotal($useTax, Cart::BOTH_WITHOUT_SHIPPING), 'currency' => $currency->id], $this->context->smarty),
            'total' => Tools::displayPriceSmarty(['price' => $totalToPay, 'currency' => $currency->id], $this->context->smarty),
    ...
    public function displayAjaxUpdate($product_addeds)
    {
        ...
        if (version_compare(_PS_VERSION_, '9.0.0', '>=')) {
            $data = (new PrestaShop\PrestaShop\Adapter\Presenter\Cart\CartPresenter())->present($this->context->cart);
        } else {
            /* @phpstan-ignore-next-line */
            $classCartPresenter = 'PrestaShop\\PrestaShop\\Adapter\\Cart\\CartPresenter';
            /* @phpstan-ignore-next-line */
            $data = call_user_func([new $classCartPresenter(), 'present'], $this->context->cart);
        }
        $assign = array(
            'cart' => $data,
    ...
    public function hookDisplayBackOfficeHeader($param)
    {
        ...
        if ($this->is17 && version_compare(_PS_VERSION_, '9.0.0', '<'))
